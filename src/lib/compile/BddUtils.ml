...
let set_reordering reorder =
  ...
  | Some i-> (
      ...
      | 5 ->
        ...
        Cudd.Man.set_sift_max_swap mgr 25;
  ...
  let rec aux i vars =
    ...
    | v :: vars -> (
        ...
        | Man.Top ->
          ...
          List.fold_left (fun acc v -> v :: Integer.add x v :: acc) [] rest
...
let generateAllSymbolicsMemoized : (Cudd.Man.v Cudd.Bdd.t -> symbolicAssignments) option ref =
  ref None

let rec addTops n lst =
  if (n = 0) then lst
  else Man.Top :: (addTops (n-1) lst)

let generateSatFast (phi : Cudd.Man.v Cudd.Bdd.t) (bounds: symbolic_var BatMap.Int.t) =
  (* Compute the assignments for the full BDD *)
  let generateAllSymbolics self generateOneSymbolic (phi : Cudd.Man.v Cudd.Bdd.t) : symbolicAssignments =
    match Bdd.inspect phi with
    | Bool false -> []
    | Bool true -> [ Collections.StringMap.empty ]
    | Ite (i, _, _) ->
      let name, start_var, end_var, ty, _ = BatMap.Int.find i bounds in
      let assignments = generateOneSymbolic (phi, start_var, end_var) in
      concat_map
        (fun (cube, phi') ->
           List.map (fun m -> Collections.StringMap.add name (cube, ty) m) (self phi'))
        assignments
  in
    ...
    | Ite (i, t, e) ->
      ...
      if i > end_var then
        ...
        *)
        ...
        let combo =
          mapAppend
            (fun (tas, phi) -> (addTops (i - start_var) (Man.True :: fillEnd t tas), phi))
            (fun (fas, phi) -> (addTops (i - start_var) (Man.False :: fillEnd e fas), phi))
...
let generateSatFastTime = ref 0.0
let toSvaluesTime = ref 0.0
...
let generateCounterExamples topology assertion (distrs: symbolic_var BatMap.Int.t) cond =
  ...
  in
  let assgns = Profile.time_profile_total generateSatFastTime (fun () -> generateSatFast a distrs) in
  Profile.time_profile_total toSvaluesTime (fun () -> symbolicAssignmentsToSvalues topology assgns)
