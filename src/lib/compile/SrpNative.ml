...
module SrpSimulation (G : Topology) : SrpSimulationSig = struct
  ...
  let simulate_solve record_fns attr_ty_id name init trans merge =
    ...
    solved :=
      (name, (Obj.magic full, Collections.TypeIds.get_elt CompileBDDs.type_store attr_ty_id))
      :: !solved;
    full
end
...
let build_solution record_fns nodes (vals, ty) =
  AdjGraph.VertexMap.map (ocaml_to_nv_value record_fns ty) vals
...
let build_solutions nodes record_fns
  ...
  let open Solution in
  {
    ...
    solves =
      List.map
        (fun (name, sol) ->
          (Var.create name, { sol_val = build_solution record_fns nodes sol; attr_ty = snd sol }))
        sols;
    nodes;
  }
