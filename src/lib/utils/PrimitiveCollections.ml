...
let printList (printer : 'a -> string) (ls : 'a list) (first : string) (sep : string)
    (last : string) =
...
let printListi (printer : int -> 'a -> string) (ls : 'a list) (first : string) (sep : string)
    (last : string) =
  let buf = Buffer.create 500 in
  let rec loop i ls =
    match ls with
