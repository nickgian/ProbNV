...
let rec strip_base_ty ty =
  match ty.typ with
  | TVar { contents = Link t } -> strip_base_ty t
  | TBool | TInt _ | TNode | TEdge -> ty.typ
...
and strip_ty ty =
  {ty with typ = strip_base_ty ty}

let tyname () = Var.fresh "a"

exception Occurs

(* Modes are not required here, we just need to check the base types *)
let occurs tvr ty =
  let rec occ tvr ty =
    match ty with
...
let rec unify info e t1 t2 : unit =
  ...
  and try_unify t1 t2 : bool =
    ...
    then true (* t1 and t2 are physically the same *)
      ...
      | TVar { contents = Link t1 }, _ ->
        try_unify t1 t2
      | _, TVar { contents = Link t2 } ->
        try_unify t1 t2
...
let substitute (ty : ty) : ty =
  ...
  let rec substitute_aux ty =
    ...
    | QVar name ->
      ...
      | None ->
        let fty = {typ = fresh_tyvar (); mode = ty.mode} in
...
(* The modes here are not necessary, but to simplify things we set them to concrete.
   TODO: maybe just set them to None *)
let op_typ op =
  match op with
  ...
  | ULess size -> 
    [concrete @@ tint_of_size size; concrete @@ tint_of_size size], concrete TBool
  ...
  let exp =
    ...
    | EOp (o, es) ->
      ...
      | Eq, [e1; e2] ->
        ...
        let m = join (OCamlUtils.oget ty1.mode) (OCamlUtils.oget ty2.mode) in
      ...
      | _ ->
        ...
        let m = List.fold_left 
                  (fun acc ty -> join (OCamlUtils.oget ty.mode) acc) 
                  (OCamlUtils.oget (List.hd tys).mode) 
                  (List.tl tys) 
        in
        texp (eop o es, mty (Some m) resty.typ, e.espan))
    | EFun { arg = x; argty; resty; body } ->
      let arg_mode = match argty with | None -> Some Concrete | Some typ -> typ.mode in
    ...
    | EApp (e1, e2) ->
      ...
      *)
      ...
      assert (fun_arg.mode = Some Concrete);
      (* If m1 is Concrete then m3 is Concrete or multi-value *)
      let _ = 
        match ty_arg.mode with
        | None | Some Symbolic ->
      ...
      (match OCamlUtils.oget ty_fun.mode with
      | Concrete -> (* If rule App-C applies *)
        (* according to App-C, m = m2 U m3 *)
        (match join_opt (OCamlUtils.oget fun_res.mode) (OCamlUtils.oget ty_arg.mode) with
         | None -> 
      ...
      | Multivalue -> (* If rule App-M applies *)
        (* modes should not matter for unification *)
        unify info e ty_fun (mty None (TArrow (ty_arg, mty None ty_res)));
        ...
        | Some Symbolic ->
          ...
          (match ty2.mode >>= fun m2 -> ty3.mode >>= fun m3 -> join_opt m2 m3 with
           | Some Concrete ->
            ...
            | Some m -> (*otherwise it is Symbolic or Multivalue, i.e. Ite-S or Ite-M*)
              texp (eif e1 e2 e3, mty (Some m) ty2.typ, e.espan)
            | None -> Console.error_position info e.espan "Join failed")
        | Some Multivalue ->
          (* Ite-M applies *)
          texp (eif e1 e2 e3, mty (Some Multivalue) ty2.typ, e.espan)) 
    | ELet (x, e1, e2) -> (* TO DO? Could traverse the term e1 again replacing
...
and infer_declaration i info env record_types d : ty Env.t * declaration =
  ...
  | DSymbolic (x, ty) ->
    Env.update env x ty, DSymbolic (x, ty)
  | DAssert e ->
  ...
  | DSolve { aty; var_names; init; trans; merge } ->
    ...
    in
    (* Computing the mode for the solutions according to Solution rule *)
    let m1 = 
    ...
    let m3 = 
      ...
      | _ -> Console.error_position info trans.espan "Merge must be of function type"
    in
    let m = join (OCamlUtils.oget m1) (join (OCamlUtils.oget m2) (OCamlUtils.oget m3)) in
    (match m with
     | Symbolic -> 
