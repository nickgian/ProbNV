...
let op_typ op =
  ...
  (* | MCreate
     ...
     | MForAll *)
  | Eq ->
    failwith (Printf.sprintf "(op_typ): %s should be handled elsewhere" (Printing.op_to_string op))
  (* LLL expressions *)
...
let rec infer_declarations_aux isHLL infer_exp i info env record_types (ds : declarations) : declarations =
  match ds with
  | [] -> []
  | d :: ds' ->
...
and infer_declaration isHLL infer_exp i info env record_types d : ty Env.t * declaration =
  ...
  | DSolve { aty; var_names; init; trans; merge } ->
    ...
    let m =
      ...
      if m1 = mtrans_arg && mtrans_arg = mtrans_res && mtrans_res = mmerge_arg1 && 
         mmerge_arg1 = mmerge_arg2 && mmerge_arg2 = mmerge_res then
        OCamlUtils.oget m1
      else
        Console.error_position info trans.espan "Modes of solution declarations do not match"
    in
    (match m with
     | Symbolic -> 
        ...
        *)
        ...
        (match OCamlUtils.oget (get_mode ty_fun) with
         | Concrete -> (* If rule App-C applies *)
           ...
           (match join_opt (OCamlUtils.oget (get_mode fun_res)) (OCamlUtils.oget (get_mode ty_arg)) with
            | None -> 
      ...
      | EIf (e1, e2, e3) ->
        ...
        (match get_mode tcond with
         ...
         | Some Concrete ->
           ...
           (match (get_mode ty2) >>= fun m2 -> (get_mode ty3) >>= fun m3 -> join_opt m2 m3 with
