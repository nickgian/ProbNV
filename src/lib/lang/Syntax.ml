...
let rec equal_base_tys ty1 ty2 =
  ...
  | (TBool | TNode | TEdge | TInt _ | TArrow _ | TVar _ | QVar _), _ ->
    false

and equal_tys ty1 ty2 =
  ty1.mode = ty2.mode && equal_base_tys ty1.typ ty2.typ

let rec equal_values ~cmp_meta (v1: value) (v2: value) =
  let b = equal_vs ~cmp_meta v1.v v2.v
  in
  if cmp_meta then
    b
