...
let sprintf msg color = T.sprintf [ T.Foreground color ] msg
...
let get_start_position (span : ProbNv_datastructures.Span.t) info =
  get_position_opt span.fname span.start info

let get_end_position (span : ProbNv_datastructures.Span.t) info =
...
let show_message_position info (span : ProbNv_datastructures.Span.t) msg color
    label =
  let border = "\n" in
