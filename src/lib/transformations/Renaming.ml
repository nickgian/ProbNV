...
let map_back bmap new_name old_name = bmap := Collections.VarMap.add new_name old_name !bmap
...
let rec alpha_convert_exp (env : Var.t Env.t) (e : exp) =
  ...
  | EIf (e1, e2, e3) ->
      eif (alpha_convert_exp env e1) (alpha_convert_exp env e2) (alpha_convert_exp env e3) |> wrap e
  | ELet (x, e1, e2) ->
      ...
      elet y e1' e2' |> wrap e
  | EBddIf (e1, e2, e3) ->
      ebddIf (alpha_convert_exp env e1) (alpha_convert_exp env e2) (alpha_convert_exp env e3)
      |> wrap e
  | EToBdd e1 -> etoBdd (alpha_convert_exp env e1) |> wrap e
  | EToMap e1 -> etoMap (alpha_convert_exp env e1) |> wrap e
  | EApplyN (e1, es) ->
      eApplyN (alpha_convert_exp env e1) (BatList.map (fun e -> alpha_convert_exp env e) es)
      |> wrap e
...
let alpha_convert_declaration bmap (env : Var.t Env.t) (d : declaration) =
  ...
  | DSolve {aty; var_names; init; trans; merge} ->
      let init, trans, merge =
        (alpha_convert_exp env init, alpha_convert_exp env trans, alpha_convert_exp env merge) in
...
let update_symbolics bmap smap =
  ...
  List.map
    (fun (s, v) -> match VarMap.Exceptionless.find s bmap with None -> (s, v) | Some k -> (k, v))
    smap
