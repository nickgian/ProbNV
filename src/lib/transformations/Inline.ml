...
(* Todo: inline/substitute patterns wherever necessary: function match on, symbolics. *)
...
let rec substitute x e1 e2 =
  ...
  | EVal _ -> e1
  | EBddIf (e3, e4, e5) ->
      ebddIf (substitute x e3 e2) (substitute x e4 e2) (substitute x e5 e2)
      |> wrap e1
  | EToBdd e3 -> etoBdd (substitute x e3 e2) |> wrap e1
  | EToMap e3 -> etoMap (substitute x e3 e2) |> wrap e1
  | EApplyN (e3, es) ->
      eApplyN (substitute x e3 e2) (BatList.map (fun e -> substitute x e e2) es)
      |> wrap e1
  | EMatch (e, bs) ->
...
and inline_branch_app cond env e2 (p, e) = (p, inline_app cond env e e2)
...
and inline_exp (cond : ty -> bool) (env : exp Env.t) (e : exp) : exp =
  ...
  | EApp (e1, e2) ->
      inline_app cond env (inline_exp env e1) (inline_exp env e2) |> wrap e
  | EIf (e1, e2, e3) ->
  ...
  | ELet (x, e1, e2) -> (
      ...
      | _ ->
          (* TODO: change this for inlining within symbolics *)
          elet x e1' (inline_exp env e2) |> wrap e )
  | ETuple es -> etuple (BatList.map (inline_exp env) es) |> wrap e
  | EMatch (e1, bs) ->
      ematch (inline_exp env e1)
        (mapBranches (fun (p, e) -> inline_branch cond env (p, e)) bs)
      |> wrap e
  | EBddIf (e1, e2, e3) ->
      ebddIf (inline_exp env e1) (inline_exp env e2) (inline_exp env e3)
      |> wrap e
...
(* This is assuming that patterns won't contain functions
   this will fail for example with an expression like:  Some (fun v -> v) *)
and inline_branch cond env (p, e) =
  let env' = remove_all env p in
  (p, inline_exp cond env' e)
...
let inline_declaration (cond : ty -> bool) (env : exp Env.t) (d : declaration) =
  ...
  | DLet (x, e, options) ->
      let e = inline_exp' env e in
      if cond (OCamlUtils.oget e.ety) && (not (hasOption "noinline" options)) then
        (Env.update env x e, None)
      else (env, Some (DLet (x, e, options)))
  | DSymbolic (x, ty, p) -> (
      match p with
...
let inline_declarations (ds : declarations) =
  inline_declarations_aux (fun _ -> true) Env.empty ds

let rec getReturnType ty =
  match ty.typ with TArrow (_, ty2) -> getReturnType ty2 | _ -> ty
